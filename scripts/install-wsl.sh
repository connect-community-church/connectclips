...
step_venv_pip() {
    ...
    log "  installing requirements.txt (cross-platform CPU base)"
    .venv/bin/pip install --quiet -r requirements.txt

    # CUDA wheels (~2 GB) are heavy — only install on boxes where the
    # NVIDIA driver actually reports a GPU. On AMD or CPU-only Linux
    # boxes this saves the download entirely; faster-whisper falls back
    # to CPU int8 cleanly without these.
    if have nvidia-smi && nvidia-smi -L 2>/dev/null | grep -q '^GPU'; then
        log "  CUDA detected — installing requirements-cuda.txt (~2 GB, ~5-15 min)"
        # onnxruntime and onnxruntime-gpu both write into the same `onnxruntime`
        # package directory — installing one over the other clobbers files
        # without updating pip metadata, leaving a broken import. Wipe both
        # before re-installing so the GPU build lands cleanly.
        .venv/bin/pip uninstall -y -q onnxruntime onnxruntime-gpu >/dev/null 2>&1 || true
        .venv/bin/pip install --quiet -r requirements-cuda.txt
    else
        log "  no CUDA driver — skipping requirements-cuda.txt; Whisper will run on CPU"
    fi
}
...
step_env_file() {
    ...
    # Pick a Whisper model the local hardware can actually run in real time.
    # large-v3 is great on NVIDIA GPUs but takes 3-5x realtime on CPU, which
    # blows the time budget for a 60-min sermon. Until the auto-tune
    # benchmarker (Phase 3 task) lands, fall back to 'small' on CPU-only
    # boxes — that's the floor for accurate Bible-reference transcription.
    if have nvidia-smi && nvidia-smi -L 2>/dev/null | grep -q '^GPU'; then
